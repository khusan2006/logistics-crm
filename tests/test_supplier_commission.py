...
def test_payment_modal_kelishuv_option_shows_narx(admin_client, db):
    ...
    # The tail is money here, not kg: this form spends the qarz down, and this is
    # the ceiling it will check the entry against (see TestKelishuvOptionShows...
    # in test_supplier_payments.py). The yuk form keeps "jami N kg".
    assert "to'lash: $1" in label
